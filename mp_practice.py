...
# use muti-processing to writing down
def print_temp (IDs, idx, NCP_len, ID_chrst, ID_metric_list, ID_AT, ID_CpG, ID_me, chip_names, chip_ID_signal):
    f = open(str(idx) + '_temp' + '.txt', 'w')
    for ID in IDs:
        chr, st = ID_chrst[ID]
        pos = st + NCP_len/2
...
def combine_all(Ncov_fname,
    ...
    def read_Ncov_file (Ncov_fname, chr_list, NCP_len):    
        ...
        for line in open(Ncov_fname):
            ...
            for i in range(len(ID_metric_list)):
                ...
                ID_metric[ID] = metric(float(counts[i]), float(counts[-1]))
    ...
    for chr in chr_list:
        ...
        #Int_dict = double_hash(ID_interval, 10000, genome_size[chr]+10)
        #Int_dict = double_hash(ID_interval, 1000, genome_size[chr]+10)
        if bs_fname or chip_fname:
            Int_dict = double_hash(ID_interval, 100000, genome_size[chr]+10)
    ...
    core_num = min(len(IDs), core_num)
    pool = mp.Pool(processes=core_num)
    step = int(round(float(len(IDs)) / core_num))
    for i in range(core_num):
        st = step*i
        ed = min(step*(i+1), len(IDs))
        pool.apply(print_temp, args=(IDs[st:ed], str(i), NCP_len, ID_chrst, ID_metric_list, ID_AT, ID_CpG, ID_me, chip_names, chip_ID_signal))
    ...
    for i in range(core_num):
        os.system("cat " + str(i) + "_temp.txt" + " >> " + out_fname + '_anot.cn')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--core',
                        ...
                        type=int,
                        default=4,
                        help='multi-processing core number')
