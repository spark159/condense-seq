...
max_pos = 5*10**8
bint_dict = Interval_dict.bin_hash({ID:(bin_size*ID, bin_size*(ID+1))
                                    for ID in range(max_pos/bin_size)},
                                   bin_size=bin_size,
                                   bin_step=bin_size,
                                   max_pos=max_pos)
...
for idx in range(max_pos/dbin_size):
    dst, ded = idx*dbin_size, (idx+1)*dbin_size
    find_binIDs = bint_dict.find_range(dst, ded)
    drange_brange[(dst, ded)] = (min(find_binIDs), max(find_binIDs)+1)
...
for exp in exp_list:
    ...
    for i in range(p_num):
        dIDs = p_dIDs[i]
        for dID in dIDs:
            chr, dst, ded = dID_chrange[dID]
            try:
                bst, bed = drange_brange[(dst, ded)]
            except:
                find_binIDs = bint_dict.find_range(dst, ded)
                bst, bed = min(find_binIDs), max(find_binIDs) + 1
            for binID in range(bst, bed):
                try:
                    score = chr_binID_score[chr][binID]
                except:
                    continue
                p_scores[i].append(score)
