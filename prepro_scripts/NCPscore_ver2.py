...
def NCP_score (fnames_list,
    ...
    for fnames in all_fnames_list:
        ...
        for fname in fnames:            
            ...
            # check data type
            if fname.endswith('_Ncov.cn'):
                data_type = 'point'
            else:
                assert fname.endswith('_bin.cn')
                data_type = 'binned'
            ...
            for line in open(fname):
                ...
                for j in range(col_st, col_ed):
                    cov = int(cols[j]) + 1 # add pesudo count
    ...
    for k in range(len(fnames_list)):
        ...
        for fname, control_fname in zip(fnames, control_fnames):
            ...
            if fname.endswith('_Ncov.cn'):
                out_fname = fname.rsplit('_Ncov.cn', 1)[0] + '_score.cn'
            else:
                assert fname.endswith('_bin.cn')
                out_fname = fname.rsplit('_bin.cn', 1)[0] + '_score.cn'
            ...
            while True:
                ...
                for i in range(col_st, col_ed):
                    count = int(test_cols[i]) + 1 # add pseudo count
                    control = int(control_cols[i]) + 1 # add pseudo count
