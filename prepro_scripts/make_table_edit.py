...
def read_BS_file (fnames, Int_dict, chr):
...
def read_chip_file (fnames, Int_dict, chr, unit='signal'):
    for fname in fnames:        
        if fname.endswith('.gz'):
            reading_file = gzip.open(fname, 'rb')
        else:
...
def read_bedgraph_file (fnames, Int_dict, chr):
    for fname in fnames:
        if fname.endswith('.gz'):
            reading_file = gzip.open(fname, 'rb')
        else:
...
def combine_all(data_fname,
    ...
    chr_intdic = {}
    if bs_fnames or chip_fnames or bg_fnames:
        for chr in chr_list:
            st_ID = chrst_ID[chr]
            ID_interval = {}
    ...
    if bs_fnames:
        for chr in chr_list:
            for bs in bs_fnames:
                ...
                fnames = bs_fnames[bs]
                ID_C, ID_meC = read_BS_file(fnames, intdict, chr)
    ...
    if chip_fnames:
        for chr in chr_list:
            for chip in chip_fnames:
                ...
                ID_signal = read_chip_file(fnames, intdict, chr)
    ...
    if bg_fnames:
        for chr in chr_list:
            for bg in bg_fnames:
                intdict = copy.deepcopy(chr_intdic[chr])
                fnames = bg_fnames[bg]
                ID_value = read_bedgraph_file(fnames, intdict, chr)
