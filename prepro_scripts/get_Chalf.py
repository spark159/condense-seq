...
def get_Chalf (fnames,
    ...
    for fname in fnames:
        ...
        for line in open(fname):
            ...
            if First:
                # set data range
                if data_type == 'point':
                    assert cols[2] == "PhysicalPosition"
                    col_st = 3
                    col_ed = len(cols)
                else:
                    assert data_type == 'binned'
                    assert cols[2] == 'Start'
                    assert cols[3] == 'End'
                    col_st = 4
                    try:
                ...
                assert len(col_ed - col_st) == len(tnums) # data col len == titration num
                s = cols[:col_st]
                s += ['C-half',
                      'height',
                      'rate',
    ...
    # summarize output
    print >> sys.stderr, "fitting failure %d/%d" % (fail_count, total_count)
    print >> sys.stderr, "fitting failure %.2f percentage" % (100*float(fail_count)/total_count)
    print >> sys.stderr, "Median R-squared of fitting %.2f" % (np.meidian(rsq_list))
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Compute the condensation point (C-half) by fitting data with a logistic function')
    ...
    if 0 not in args.tnums:
        print >> sys.stderr, "input control data should be included (titration number is zero)"

    # check at least 3 titration points for fitting
    if len(args.tnums) < 3:
        print >> sys.stderr, "Need more data points for fitting"
