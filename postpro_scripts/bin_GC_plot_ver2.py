...
# experiment list (rep, cell, sample, agent, tnum)
exp_list = [(0, 'H1', 'NCP', 'sp', i) for i in [4,8]]
...
for exp in exp_list:
    ...
    print "%s %s %s %d %drep" % (cell, sample, agent, tnum, rep)
    
    # load score bin file
    fname = path + '/' + '_'.join([cell, sample, agent, str(rep) + 'rep' + deep_note, str(int(bin_size/1000.0)) + 'kb', dtype]) + '.cn'
    field_name = '_'.join([cell, sample, agent, str(tnum), str(rep) + 'rep' + deep_note])
    chr_binID_score = read_bin_score(fname, bin_size)[field_name]

    # load bin file
    fname = path + '/' + '_'.join([cell, sample, agent, str(0), str(rep) + 'rep' + deep_note, str(int(bin_size/1000.0)) + 'kb', 'bin']) + '.cn'
    ...
    for exp in exp_list:
        ...
        for i in range(len(names)-1):
            for j in range(i+1, len(names)):
                ...
                fig = plt.figure()
                # scatter plot
                plt.plot(data1, data2, '.', markersize=3, alpha=0.2)
                ...
                plt.savefig("%sVS%s_%s_%s_%s_%d_%drep_%s.png"
                            % (name1, name2, cell, sample, agent, tnum, rep, dtype),
                            bbox_inches='tight')
                plt.close()
