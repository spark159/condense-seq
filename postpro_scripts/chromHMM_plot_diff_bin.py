...
#fname1 = path + '_'.join([cell1, sample1, agent, str(int(bin_size/1000.0)) + 'kb']) + '_score.cn'
#fname2 = path + '_'.join([cell2, sample2, agent, str(int(bin_size/1000.0)) + 'kb']) + '_score.cn'
#fname3 = path + '_'.join([cell3, sample3, agent, str(int(bin_size/1000.0)) + 'kb']) + '_score.cn'

fname1 = path + '_'.join([cell1, sample1, agent, str(int(bin_size/1000.0)) + 'kb']) + '_zscore.cn'
fname2 = path + '_'.join([cell2, sample2, agent, str(int(bin_size/1000.0)) + 'kb']) + '_zscore.cn'
fname3 = path + '_'.join([cell3, sample3, agent, str(int(bin_size/1000.0)) + 'kb']) + '_zscore.cn'


chr_binID_score1 = read_bin_score(fname1)['-'.join([cell1, sample1, agent, '8']) + '.bam']
chr_binID_score2 = read_bin_score(fname2)['-'.join([cell2, sample2, agent, '8']) + '.bam']
chr_binID_score3 = read_bin_score(fname3)['-'.join([cell3, sample3, agent, '8']) + '.bam']

chr_state_intervals = read_chromHMM(mCD8T_HMM_fname, chr_choice=chr_list, change=mCD8T_name_dict)

#state_scores1 = categorize (chr_state_intervals, chr_binID_score1)
#state_scores2 = categorize (chr_state_intervals, chr_binID_score2)
#state_scores3 = categorize (chr_state_intervals, chr_binID_score3)
...
for chr in chr_list:
    binID_score1 = chr_binID_score1[chr]
    binID_score2 = chr_binID_score2[chr]
    binID_score3 = chr_binID_score3[chr]
    
    common_binIDs = list(set(binID_score1) & set(binID_score2) & set(binID_score3))

    for binID in common_binIDs:
        dscore1 = binID_score2[binID] - binID_score1[binID]
        dscore2 = binID_score3[binID] - binID_score1[binID]

        if chr not in chr_binID_dscore1:
            chr_binID_dscore1[chr] = {}
        if chr not in chr_binID_dscore2:
            chr_binID_dscore2[chr] = {}
        chr_binID_dscore1[chr][binID] = dscore1
        chr_binID_dscore2[chr][binID] = dscore2

state_dscores1 = categorize (chr_state_intervals, chr_binID_dscore1)
state_dscores2 = categorize (chr_state_intervals, chr_binID_dscore2)

multi_boxplot ([state_dscores1, state_dscores2],
               axhline=0,
               ylabel = "$\Delta$" + ylabel,
               title=None,
               keys=mCD8T_states,
               labels = ['+inht-WT', 'KO-WT'],
               colors = ['tab:orange', 'tab:green'],
               rotation=75,
               legend_loc='lower right', 
               note=note + '_' + "$\Delta$" + ylabel)
...
#sxingle_boxplot (state_scores2, keys=GM_states, rotation=75, ylim=[0, 8], note='GM')
