...
path = ""
fname = "H1_NCP_sp_chr1_167win25step_anot.cn"
jump = 10
ID_chr, ID_pos, name_ID_value = load_file.read_anot_file(path + fname, jump=jump)
...
names = ['work/2021_06_07_H1_sp_detail/H1-NCP-sp-8', 'ATcontent']
step_size = 25*jump
...
for name in names:
    ...
    if name.startswith('work') or name.startswith('data'):
        name = "Condensability"
        alpha = 1
    elif name == 'ATcontent':
        name = 'AT content'
    ...
    #acf = np.exp(acf)
    plt.plot(X[1:], acf[1:], label=name, alpha=alpha, lw=2)
...
plt.savefig('ACF_condensability.svg', format='svg', bbox_inches='tight')
#plt.show()
plt.close()
