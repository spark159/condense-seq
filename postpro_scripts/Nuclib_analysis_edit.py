...
def Nuclib_analysis (sort_fnames,
                     table_fname,
                     index_fname,
                     agent_tfname,
                     exclude_IDs,
    ...
    for rep in range(repnum):
        ...
        for agent in agent_tnum_ID_count:
            ...
            for tnum in tnums:
                ...
                # get total count of input
                # define target IDs
                if tnum == 0:
                    total_control = 0
                    ID_control = tnum_ID_count[tnum]
                    target_IDs = []
                    for ID in ID_control:
                        if ID in exclude_IDs: # skip the excluded IDs
                            continue
                        control = ID_control[ID]
                        if control <=0: # skip when input data is empty
                            continue
                        control +=1 # add pseudocount
                ...
                for ID in target_IDs:
                    try:
                        ...
                        count = 0
                    count +=1 # add pseudocount
                    total += count

                # compute fold-change and survival probability
                for ID in target_IDs:
                    control = ID_control[ID]
                    control +=1 # add pseudocount
        ...
        agent_ID_score = {}
        agent_ID_logistic = {}
        for agent in agent_ID_titration:
            ID_titration = agent_ID_titration[agent]
            IDs = sorted(ID_titration.keys())
            rsq_list = []
            total_count, fail_count = 0, 0
            for ID in IDs:
                X = ID_titration[agent][ID]['conc']
                Y = ID_titration[agent][ID]['survival']
            ...
            print >> sys.stderr, "logitic fitting: %s, rep-%d" % (agent, rep+1)
            print >> sys.stderr, "fitting failure %d/%d (%.2f %%)" % (fail_count,
                                                                      total_count,
                                                                      100*float(fail_count)/total_count)
            print >> sys.stderr, "Median R-squared of fitting %.2f" % (np.median(rsq_list))
            print >> sys.stderr, ""
    ...
    for rep in range(repnum):
        ...
        for agent in agent_ID_titration:
            ID_titration = agent_ID_titration[agent]
            IDs = sorted(ID_titration.keys())
            concs = ID_titration[IDs[0]]['conc']
            concs_string = ['[%s] %f %s' % (agent, conc, agent_unit[agent]) for conc in concs]
            
            f = open("%s_%s_rep-%d_survival.txt" % (outfname, agent, rep+1), 'w')
    ...
    for agent in agent_list:
        common_IDs = set([])
        for rep in range(repnum):
            IDs = set(rep_agent_ID_score[rep][agent].keys())
            common_IDs |= IDs
        common_IDs = sorted(list(common_IDs))
        
        f = open("%s_%s_score.txt" % (outfname, agent), 'w')
    ...
    for agent in agent_list:
        common_IDs = set([])
        for rep in range(repnum):
            IDs = set(rep_agent_ID_logistic[rep][agent].keys())
            common_IDs |= IDs
        common_IDs = sorted(list(common_IDs))

        f = open("%s_%s_logistic.txt" % (outfname, agent), 'w')
...
if __name__ == '__main__':
    ...
    exclude_IDs = [71, 78, 113, 116, 117] #BC with PstI site or freeDNA

    outfname = 'PTMlib'
    
    Nuclib_analysis (sort_fnames,
                     table_fname,
                     index_fname,
                     agent_tfname,
                     exclude_IDs,
