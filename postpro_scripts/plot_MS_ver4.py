    ...
    for rep in rep_list:
        ...
        for peptide in sample_peptide_ptm_ratio['input-' + str(rep)].keys():
            ...
            for ptm in sample_peptide_ptm_ratio['input-' + str(rep)][peptide]:
                ...
                #if sum([input_ratio, sup_ratio]) <= 0:
                #    continue
...
#peptides = sample_peptide_ptm_ratio['input-0'].keys()

for cdt in cdts:
    peptides = sample_peptide_ptm_fold[cdt +'-0'].keys()
    for peptide in peptides:
        ...
        ptm_ratios, ptm_folds = {}, {}
        for rep in reps:
            input_sample = 'input-%d' % (rep)
            cdt_sample = '%s-%d' % (cdt, rep)
            input_ptm_ratio = sample_peptide_ptm_ratio[input_sample][peptide]
            cdt_ptm_fold = sample_peptide_ptm_fold[cdt_sample][peptide]

            for ptm in cdt_ptm_fold:
                ratio = input_ptm_ratio[ptm]
                fold = cdt_ptm_fold[ptm]

                if ptm not in ptm_ratios:
                    ptm_ratios[ptm] = []
                ptm_ratios[ptm].append(ratio)
        ...
        ref_ptm = '_'.join([histone, str(st), str(ed)]) + ' unmod'
        ref_ratios = ptm_ratios[ref_ptm]
        ref_folds = ptm_folds[ref_ptm]

        assert len(ref_ratios) == len(ref_folds)
        ...
        wmean_ref_fold = weighted_mean(ref_folds, ref_ratios) # weighted mean

        if cdt not in cdt_peptide_ptm_mfold:
            cdt_peptide_ptm_mfold[cdt] = {}
        if peptide not in cdt_peptide_ptm_mfold[cdt]:
            cdt_peptide_ptm_mfold[cdt][peptide] = {}
        cdt_peptide_ptm_mfold[cdt][peptide][ref_ptm] = wmean_ref_fold
        
        # compare each ptm to the reference data
        ptms = list(set(ptm_folds.keys()) - set([ref_ptm]))        
        for ptm in ptms:
            target_ratios = ptm_ratios[ptm]
            target_folds = ptm_folds[ptm]

            assert len(target_ratios) == len(target_folds)

            #if len(target_ratios) <= 1:
            #    continue
            if sum(target_ratios) <=0:
                continue
            
            mean_target_fold = np.mean(target_folds) # uniform mean
            wmean_target_fold = weighted_mean(target_folds, target_ratios) # weighted mean

            if cdt not in cdt_peptide_ptm_mfold:
                cdt_peptide_ptm_mfold[cdt] = {}
            if peptide not in cdt_peptide_ptm_mfold[cdt]:
                cdt_peptide_ptm_mfold[cdt][peptide] = {}
            cdt_peptide_ptm_mfold[cdt][peptide][ptm] = wmean_target_fold
            
            #delta = mean_target_fold - mean_ref_fold
            delta = wmean_target_fold - wmean_ref_fold

            if len(ref_folds) <= 1 or len(target_folds) <= 1:
                #pvalue = 0.99
                pvalue = 'NA'
                logp = 'NA'
            else:
            ...
            if cdt not in cdt_peptide_ptm_sig:
                cdt_peptide_ptm_sig[cdt] = {}
            if peptide not in cdt_peptide_ptm_sig[cdt]:
                cdt_peptide_ptm_sig[cdt][peptide] = {}
            cdt_peptide_ptm_sig[cdt][peptide][ptm] = logp

# write down fold change
samples = []
for rep in reps:
    for cdt in cdts:
        samples.append('%s-%d' % (cdt, rep+1))
for cdt in cdts:
    samples += ['log2 fold-change(%s/input)' % (cdt),
                'delta fold-change (ptm - umod)'
                'significance (-log10 p-value)']

f = open('histone_ptm_fold_change.txt', 'w')
print >> f, '\t' + '\t'.join(samples)

First = True
for line in open('Sangwoo_histone_ratios_092122.txt'):
    line = line.strip()
    if not line:
        continue
    if First:
    ...
    for rep in reps:
        for cdt in cdts:
            sample = '%s-%d' % (cdt, rep)
            try:
                value = sample_peptide_ptm_fold[sample][peptide][ptm]
    ...
    for cdt in cdts:
        try:
            mfold = cdt_peptide_ptm_mfold[cdt][peptide][ptm]
            mfold = str(round(mfold, 6))
        except:
            mfold = 'NA'
        values.append(mfold)
...
for cdt in cdt_peptide_ptm_delta:
    ...
    min_delta, max_delta = -1.5*np.std(deltas), 1.5*np.std(deltas)
    min_sig, max_sig = min(sigs), max(sigs)
    
    up_cmap = mpl.cm.get_cmap("Reds")
    down_cmap = mpl.cm.get_cmap("Blues_r")
    bi_cmap = mpl.cm.get_cmap("seismic")

    #up_cmap = mpl.cm.get_cmap("YlOrRd")
    #down_cmap = mpl.cm.get_cmap("YlGnBu_r")
    ...
    for peptide in cdt_peptide_ptm_delta[cdt]:
        ...
        for ptm in ptms:
            ...
            color = bi_cmap(rescale(delta, min_delta, max_delta, 0.0, 1.0))
            size = rescale(sig, min_sig, max_sig, 10, 30)
            #size = 10
            colors.append(color)
            sizes.append(size)
        ...
        #            bbox_inches='tight')
        plt.savefig('%s_delta.svg' % (peptide),
                    format='svg',
                    bbox_inches='tight')
    ...
    plt.subplot(1,2,1)
    cbar = mpl.colorbar.ColorbarBase(plt.gca(), cmap='seismic')
    cbar.ax.set_yticks([0, 1])
    delta_label = round(max_delta, 2)
    cbar.ax.set_yticklabels([str(-delta_label), str(delta_label)], fontsize=8)
    cbar.ax.set_ylabel('$\\Delta$' +' log2 fold-change', rotation=-90, va="bottom", fontsize=8)
    plt.tight_layout()
    #plt.savefig('MS_cbar.svg', format='svg', bbox_inches='tight')
    plt.savefig('MS_cbar.png', bbox_inches='tight')
    plt.close()

    # save circle size legend
    fig = plt.figure(figsize=(1,3))
    examples = np.linspace(-np.log10(10**-10), -np.log10(1-10**-10), 5)
    labels = []
    for i in range(len(examples)):
        example = examples[i]
        labels.append(round(example, 3))
        size = rescale(example, min_sig, max_sig, 10, 30)
        plt.plot([0],[i],
                 marker='o',
                 markersize=size,
                 mfc='black',
    ...
    #plt.savefig('circle_legend.png', bbox_inches='tight')
    plt.savefig('circle_legend.svg',
                format='svg',
                bbox_inches='tight')
    plt.close()
...
cdt_ptm_mfold = {}
for cdt in cdt_peptide_ptm_mfold:
    peptide_ptm_mfold = cdt_peptide_ptm_mfold[cdt]
    for peptide in peptide_ptm_mfold:
        for ptm in peptide_ptm_mfold[peptide]:
            mfold = peptide_ptm_mfold[peptide][ptm]
            if cdt not in cdt_ptm_mfold:
                cdt_ptm_mfold[cdt] = {}
            assert ptm not in cdt_ptm_mfold
            cdt_ptm_mfold[cdt][ptm] = mfold
...
def plot_corr (sample_ptm_value,
               sample_list=None,
               name_list = None,
               figsize = (7,6),
               note=''):
    ...
    for i in range(len(sample_list)):
        for j in range(len(sample_list)):
            ...
            if i > j:
                ...
                s = name_list[i]
                axes[i,j].text(len(sample_list)/2,
                               len(sample_list)/2,
                               s,
                               ha="center",
                               va="center",
...
plot_corr(sample_ptm_fold,
          sample_list = ['sup-%d' % (i) for i in range(4)],
          name_list = ['Rep #%d' % (i+1) for i in range(4)],
          figsize = (3,2.5),
          note='old')
...
def plot_hcluster (name_state,
    ...
    name_list = list(name_state.keys())
    data = []
...
plot_hcluster(sptm_state, figsize = (5,10), xticklabels=sample_list)
...
plot_hcluster(sptm_state0,
              xticklabels = conditions,
              note='first')

plot_hcluster(sptm_state1,
              xticklabels = conditions,
              note='old_average')

plot_hcluster(sptm_state2,
              xticklabels = conditions,
              note='total_average')
