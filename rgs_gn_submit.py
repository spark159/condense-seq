...
if __name__ == '__main__':
    ...
    class GN (DirectedGraph):

        def condensation (self):
            SCC_list = self.Tarjan()
            
        
        def get_potential (self):
            potential = 0.0
            SCC_list = self.Tarjan()
            for i in range(len(SCC_list)-1):
                for j in range(i+1, len(SCC_list)):
                    SCC1, SCC2 = SCC_list[i], SCC_list[j]
                    weight = 0.0
                    for node1 in SCC1:
                        for node2 in SCC2:
                            try:
                                weight += self.node_info[node1]['next'][node2]
                            except:
                                try:
                                    weight += self.node_info[node2]['next'][node1]
                                except:
                                    pass
