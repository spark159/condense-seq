...
occ_limit = 10 # occupancy number limit
pvden = 10**-5 # particle volume density
N = len(ID_score) # total particle number used for this simulation
volume = pvden*N # volume of solution (in the unit of particle volume) used for this simulation 


def collision_prob (volume):
    return 1.0 / float(volume)

def bonding_prob (energy):
    return np.exp(-energy) / float(1.0 + np.exp(-energy))
