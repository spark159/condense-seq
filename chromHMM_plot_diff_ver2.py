...
state_scores_list = [{} for i in range(len(exp_list))]
for chr_name in chr_list:

    print "Processing %s" % (chr_name)
    ...
    for state, intervals in state_intervals.items():
        for i in range(len(intervals)):
            sID = state + ':' + str(i)
            sID_interval[sID] = intervals[i]

    HMM_dict = Interval_dict.double_hash(sID_interval, 10000, 250000000)
    ...
    for i in range(len(exp_list)):
        ...
        state_scores = state_scores_list[i]
        for ID , pos in ID_pos.items():
            find_sIDs = HMM_dict.find(pos)
            if len(find_sIDs) != 1:
                continue
            state = find_sIDs[0].split(':')[0]
            score = ID_score[ID]
...
for exp, state_scores in zip(exp_list, state_scores_list):
